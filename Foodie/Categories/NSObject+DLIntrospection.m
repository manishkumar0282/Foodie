    ...
    for (int i = 0; i < methodCount; i++) {
        [methodsDescription addObject:
         [NSString stringWithFormat:@"%@ (%@)%@",
          instance ? @"-" : @"+",
//#warning return correct type  - Navi suppressed warning.
          @"void",
...
+ (NSString *)formattedPropery:(objc_property_t)prop {
    ...
    if (attributes[@"R"]) {[attrsArray addObject:@"readonly"];}
    if (attributes[@"G"]) {
    ...
    [property appendFormat:@"(%@) %@ %@",
     ...
     [NSString decodeType:[attributes[@"T"] cStringUsingEncoding:NSUTF8StringEncoding]],
